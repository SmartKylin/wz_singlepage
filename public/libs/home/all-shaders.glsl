...
void main() {
    ...
    float t = pow(v_texcoord.y, 0.15);
//    float t = sin(v_texcoord.y);
//    gl_FragColor.rgb = mix(color0, color1, t) + 0.3*smoke;
    gl_FragColor.rgb = mix(color0, color1, t);
    gl_FragColor.a = 1.0;
}
...
void main() {
    ...
    gl_FragColor.a = (1.0 - pow(time, 7.0)) * (v_alpha * time)*ccalpha;
}
